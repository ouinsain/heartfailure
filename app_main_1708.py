...
with tab1:
    ...
    #'RestingECG'
    x7 = st.selectbox('RestingECG', RestingECG_encoder.classes_)
    x7 = RestingECG_encoder.transform([x7])[0]

    #'MaxHR'
    x8 = st.slider('MaxHR', 60, 202, 90)

    #'ExerciseAngina'
    x9 = st.radio('Exercise Angina', ExerciseAngina_encoder.classes_)
    x9 = ExerciseAngina_encoder.transform([x9])[0]

    #'Oldpeak'
    x10 = st.slider('Oldpeak', -2.60, 6.20, 0.88)

    #'ST_Slope'
    x11 = st.selectbox('Slect ST_Slope', ST_Slope_encoder.classes_)
...
with tab2:
    ...
    fig = px.Figure(data=[
        px.Bar(name = 'Decision Tree',
               x = x,
               y = evaluations.loc['Decision Trees']),
